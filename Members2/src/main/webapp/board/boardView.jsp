...
<body>
	...
	<div id="container">
		<section id="board_detail">
			...
			<tbody>
				<tr>
					<td><input type="text" name="memberId" readonly=readonly value="${board.title}">
				</tr>
				<tr>
					<td><textarea rows='8' cols='100' readonly=readonly>${board.content}</textarea>
					</td>
				</tr>
				<tr>
					<td><c:out value="글쓴이:${board.memberId}" /> 
					
					<c:if test="${board.modifyDate != null}">
					...
					<tr>
						<td><c:if test="${board.memberId == sessionId}">
								...
								<a href="/deleteBoard.do?bnum=${board.bnum}"
									onclick="return cofirm('정말로 삭제하시겠습니까?')">
									<button type="button">삭제</button></a>
							</c:if> <a href="/boardList.do"><button type="button">목록</button></a>
						</td>
					</tr>
				</tbody>
			</table>
